...
const Blog = ({ data, pageContext, location }) => {
  ...
  return (
    <div>
      <Seo name='Blog Academia Cultural' title='Blog Academia Cultural' type='blog' url={location?.href} />
      <Navigator
      ...
      <main>
        <div className={`${pageContext.currentPage <= 0 ? '' : 'hidden'} mx-4 2xl:mx-80`}>
          <div className='relative container mx-auto bg-green-500 flex items-center justify-between border border-transparent rounded-xl'>
            <h1 className='absolute left-14 top-16 text-xl sm:text-4xl text-white  font-bold pb-2'>Dicas e conteúdos exclusivos</h1>
            <h1 className='absolute left-14 top-28 text-xl sm:text-4xl text-white font-bold pb-2'>com os nossos professores!</h1>
            <p className='absolute left-14 top-40 text-lg sm:text-xl text-white font-medium py-2'>Compartilhe saúde com seus amigos!</p>
        ...
        <div className={`${pageContext.currentPage <= 0 ? '' : 'hidden'} mx-4 2xl:mx-80`}>
          <BlogCategories>
            {data.categories.edges.map(({ node }) => (
              <BlogCategoriesItem category={node.category} image={node.image.gatsbyImageData} slug={`/blog${node.slug}`} />
            ))}
          </BlogCategories>
        </div>
        <BlogColunCards title='Postagens mais recentes' description='Saúde, disposição, felicidade e muito mais.'>
...
export const pageQuery = graphql`
  query($skip: Int!, $limit: Int!) {
    posts: allContentfulPost(limit: $limit, skip: $skip, filter: { visible: { eq: true } }, sort: { order: DESC, fields: createdAt }) {
      edges {
        node {
          title
          slug
