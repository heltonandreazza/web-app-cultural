...
const buildOptions = (currentUrl) => {
  const options = {
    ...
    renderNode: {
      ...
      [BLOCKS.PARAGRAPH]: (node, children) => <p className='mt-8 text-xl text-gray-500 leading-8'>{children}</p>,
      ...
      [BLOCKS.QUOTE]: (node, children) => (
        <blockquote className='border-l-4 pl-6'>
          <p className='mt-8 text-xl text-gray-900 italic leading-8'>{node.content[0].content[0].value}</p>
        </blockquote>
      ),
      ...
      [BLOCKS.EMBEDDED_ASSET]: (node, children) => (
        ...
        <GatsbyImage className='rounded-xl mt-8' image={node.data.target?.fields?.gatsbyImageData} />
      ),
    },
  }
  return options
...
const Post = ({ data: { post, posts } = {}, path, location }) => {
...
export const pageQuery = graphql`
  query($slug: String!) {
    ...
    posts: allContentfulPost(limit: 4, filter: { visible: { eq: true } }, sort: { order: DESC, fields: createdAt }) {
      edges {
        node {
          title
          slug
